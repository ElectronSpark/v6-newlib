/**
 * @file kqueuetest.c
 * @brief Comprehensive test suite for the kqueue subsystem (newlib version)
 *
 * Categories:
 ...
 *   3. Stress tests
 *
 * NOTE: This file uses standard C / POSIX headers via newlib exclusively.
 *       No xv6 kernel headers are included.
 */
...
static void test_pipe_read(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, fds[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 42);
...
static void test_pipe_write(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, fds[1], EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, 55);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(fds[0]); close(fds[1]); close(kq); return;
    }
...
static void test_timer_oneshot(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 1, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT, 0, 100, 99);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(kq); return;
    }
...
static void test_timer_periodic(void) {
    ...
    EV_SET(&ev, 10, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_CLEAR, 0, 50, 0);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(kq); return;
    }
    ...
    EV_SET(&ev, 10, EVFILT_TIMER, EV_DELETE, 0, 0, 0);
...
static void test_proc_fork(void) {
    ...
    pid_t pid = getpid();
    struct kevent ev;
    EV_SET(&ev, pid, EVFILT_PROC, EV_ADD | EV_ENABLE, NOTE_FORK | NOTE_EXIT, 0, 77);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(kq); return;
    }
...
static void test_pipe_eof(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, fds[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 0);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(fds[0]); close(fds[1]); close(kq); return;
    }
...
static void test_ev_disable_enable(void) {
    ...
    EV_SET(&ev, fds[0], EVFILT_READ, EV_ADD | EV_DISABLE, 0, 0, 0);
    kevent_register(kq, &ev, 1);

    write(fds[1], "data", 4);
    /* Poll with short timeout — should see nothing since knote is disabled */
    struct kevent out;
    ...
    /* Now re-enable */
    EV_SET(&ev, fds[0], EVFILT_READ, EV_ENABLE, 0, 0, 0);
    kevent_register(kq, &ev, 1);
...
static void test_ev_delete(void) {
    ...
    /* Delete it */
    EV_SET(&ev, fds[0], EVFILT_READ, EV_DELETE, 0, 0, 0);
    kevent_register(kq, &ev, 1);
...
static void test_multi_event(void) {
    ...
    struct kevent evs[2];
    EV_SET(&evs[0], fds1[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 1);
    EV_SET(&evs[1], fds2[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 2);
...
static void test_poll_no_event(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 100, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT, 0, 10000, 0);
    kevent_register(kq, &ev, 1);
    ...
    /* Clean up timer */
    EV_SET(&ev, 100, EVFILT_TIMER, EV_DELETE, 0, 0, 0);
    kevent_register(kq, &ev, 1);
    close(kq);
}
...
static void test_neg_bad_filter(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 0, -99, EV_ADD, 0, 0, 0);  /* -99 is not a valid filter */
...
static void test_neg_delete_nonexistent(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 12345, EVFILT_READ, EV_DELETE, 0, 0, 0);
    kevent_register(kq, &ev, 1);
...
static void test_neg_read_bad_fd(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 999, EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 0);
    int ret = kevent_register(kq, &ev, 1);
...
static void test_neg_timer_zero(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 50, EVFILT_TIMER, EV_ADD | EV_ENABLE, 0, 0, 0);  /* data=0 */
    int ret = kevent_register(kq, &ev, 1);
    if ((ret == 0 && (ev.flags & EV_ERROR)) || ret < 0)
        TEST_PASS(name);
    else
...
static void test_neg_proc_bad_pid(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 99999, EVFILT_PROC, EV_ADD | EV_ENABLE, NOTE_EXIT, 0, 0);
    int ret = kevent_register(kq, &ev, 1);
    if ((ret == 0 && (ev.flags & EV_ERROR)) || ret < 0)
        TEST_PASS(name);
    else
...
static void test_neg_register_zero(void) {
    ...
    int ret = kevent_register(kq, NULL, 0);
    if (ret == 0)
        TEST_PASS(name);
    else
...
static void test_neg_double_close(void) {
    ...
    (void)ret;
    TEST_PASS(name);  /* not crashing is the main requirement */
}
...
static void test_neg_register_no_flags(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, 0, EVFILT_TIMER, 0, 0, 100, 0);  /* flags = 0 */
    int ret = kevent_register(kq, &ev, 1);
...
static void test_stress_many_timers(void) {
    ...
    for (int i = 0; i < ntimers; i++) {
        struct kevent ev;
        EV_SET(&ev, (uintptr_t)(i + 100), EVFILT_TIMER,
               EV_ADD | EV_ENABLE | EV_ONESHOT, 0, 100 + i * 50, (uintptr_t)i);
        if (kevent_register(kq, &ev, 1) < 0) {
            TEST_FAIL(name, "kevent_register failed");
            close(kq);
            return;
...
static void test_stress_many_pipes(void) {
    ...
    for (int i = 0; i < npipes; i++) {
        ...
        struct kevent ev;
        EV_SET(&ev, pfds[i][0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, (uintptr_t)i);
        kevent_register(kq, &ev, 1);
    }
...
static void test_stress_register_delete_churn(void) {
    ...
    for (int i = 0; i < 100; i++) {
        struct kevent ev;
        EV_SET(&ev, fds[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, (uintptr_t)i);
...
static void test_stress_timer_pipe_interleave(void) {
    ...
    struct kevent evs[2];
    EV_SET(&evs[0], fds[0], EVFILT_READ, EV_ADD | EV_ENABLE, 0, 0, 1);
    EV_SET(&evs[1], 200, EVFILT_TIMER, EV_ADD | EV_ENABLE | EV_ONESHOT, 0, 100, 2);
    kevent_register(kq, evs, 2);
...
static void test_stress_proc_multi_fork(void) {
    ...
    pid_t pid = getpid();
    struct kevent ev;
    EV_SET(&ev, pid, EVFILT_PROC, EV_ADD | EV_ENABLE | EV_CLEAR, NOTE_FORK, 0, 0);
    if (kevent_register(kq, &ev, 1) < 0) {
        TEST_FAIL(name, "kevent_register failed");
        close(kq); return;
    }
...
static void test_stress_pipe_roundtrip(void) {
    ...
    struct kevent ev;
    EV_SET(&ev, c2p[0], EVFILT_READ, EV_ADD | EV_ENABLE | EV_CLEAR, 0, 0, 0);
    kevent_register(kq, &ev, 1);
